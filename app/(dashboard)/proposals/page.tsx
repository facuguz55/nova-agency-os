...
const STATUS_BADGE: Record<string, string> = {
  draft:    'bg-[#334155]/30 text-[#64748b] border-[#334155]/40',
    ...
    <>
      ...
      <div className="flex-1 flex overflow-hidden bg-grid">
        ...
        <div className="w-72 shrink-0 border-r border-[#1e2f4a] flex flex-col">
          <div className="p-3 border-b border-[#1e2f4a]">
            <p className="text-[10px] text-[#334155] uppercase tracking-widest">Propuestas</p>
          </div>
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 gap-3 px-4">
                <FileSignature size={28} className="text-[#1e2f4a]" />
                <p className="text-xs text-[#334155] text-center">Sin propuestas. Generá una con IA.</p>
              </div>
            ) : proposals.map(p => (
              <div
                key={p.id}
                ...
                className={cn(
                  'p-3 border-b border-[#1e2f4a]/50 cursor-pointer transition-colors',
                  selected?.id === p.id
                    ? 'bg-[#ff8c42]/5 border-l-2 border-l-[#ff8c42]'
                    : 'hover:bg-white/[.02]',
                )}
              >
                ...
                <div className="flex items-center justify-between mt-1.5 gap-2">
                  ...
                  <span className="text-[10px] text-[#334155] shrink-0">{formatRelative(p.created_at)}</span>
                </div>
                {p.clients?.name && (
                  <p className="text-[10px] text-[#475569] mt-1 truncate">{p.clients.name}</p>
                )}
              </div>
            ))}
          </div>
            ...
            <div className="max-w-3xl space-y-5">
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <div className="relative">
                    ...
                    {statusOpen && (
                      <div className="absolute right-0 top-full mt-1 z-20 bg-[#0e1a2e] border border-[#1e2f4a] rounded-xl overflow-hidden shadow-xl min-w-[130px]">
                        {STATUS_ACTIONS.map(s => (
                          <button
                            key={s}
                            onClick={() => changeStatus(selected.id, s)}
                            className="flex items-center gap-2 w-full px-3 py-2 text-xs text-[#94a3b8] hover:bg-white/[.04] hover:text-white transition-colors"
                          >
                            {STATUS_LABEL[s]}
                          </button>
                        ))}
              ...
              <div className="grid grid-cols-3 gap-3">
                {selected.clients?.name && (
                  <div className="bg-[#0e1a2e] border border-[#1e2f4a] rounded-xl p-3">
                    <p className="text-[10px] text-[#334155] uppercase tracking-widest mb-1">Cliente</p>
                    <p className="text-sm text-white font-medium">{selected.clients.name}</p>
                    {selected.clients.industry && <p className="text-[10px] text-[#475569] mt-0.5">{selected.clients.industry}</p>}
                  </div>
                )}
                {selected.service && (
                  <div className="bg-[#0e1a2e] border border-[#1e2f4a] rounded-xl p-3">
                    <p className="text-[10px] text-[#334155] uppercase tracking-widest mb-1">Servicio</p>
                    <p className="text-sm text-white font-medium">{selected.service}</p>
                  </div>
                )}
                {selected.amount && (
                  <div className="bg-[#0e1a2e] border border-[#1e2f4a] rounded-xl p-3">
                    <p className="text-[10px] text-[#334155] uppercase tracking-widest mb-1">Monto</p>
              ...
              {selected.content ? (
                <div className="bg-[#080f1e] border border-[#1e2f4a] rounded-xl p-5">
                  <p className="text-sm text-[#94a3b8] whitespace-pre-wrap font-sans leading-relaxed">{selected.content}</p>
                </div>
              ) : (
                <div className="bg-[#080f1e] border border-[#1e2f4a]/50 border-dashed rounded-xl p-8 text-center">
                  <p className="text-sm text-[#334155]">Esta propuesta no tiene contenido todavía.</p>
                </div>
              )}
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center h-full gap-3">
              <FileSignature size={40} className="text-[#1e2f4a]" />
              <p className="text-sm text-[#334155]">Seleccioná una propuesta</p>
            </div>
          )}
        </div>
      </div>
      ...
      <Modal open={showModal} onClose={() => { setShowModal(false); setForm(EMPTY_FORM) }} title="Nueva propuesta" size="lg">
        <div className="space-y-4">
          ...
          <div className="border-t border-[#1e2f4a] pt-4">
            ...
            <div className="flex gap-3">
              <Button
                ...
                disabled={generating || !form.client_id || !form.service}
              >
                {generating
          ...
          <div className="border-t border-[#1e2f4a] pt-4 space-y-3">
            <p className="text-[10px] text-[#334155] uppercase tracking-widest">Creación manual</p>
            <Input
              label="Título *"
              value={form.title}
              onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
              placeholder="Propuesta SEO — Empresa XYZ"
            />
            <div className="flex gap-3">
              <Button
                variant="secondary"
