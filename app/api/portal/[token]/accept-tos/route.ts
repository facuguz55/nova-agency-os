...
export async function POST(req: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  // Verificar PIN antes de registrar aceptación
  ...
  // Registrar aceptación con timestamp
  const { error: updateError } = await supabase
    .from('client_portals')
    .update({ tos_accepted_at: new Date().toISOString() })
    .eq('id', data.id)

  if (updateError) return NextResponse.json({ ok: false, error: 'Error al guardar' }, { status: 500 })
